...
@Component({
  selector: 'app-copy-fileds-values',
  templateUrl: './copy-fileds-values.component.html',
  styleUrls: ['./copy-fileds-values.component.scss']
})
export class CopyFiledsValuesComponent extends APIService implements OnInit {
  ...
  postNew($item: any) {
    ...
    this.getForm("http://localhost:8666/internal/copy-fields-values/" + $item.formId + "/" + $item.subToCopy + "/" + $item.subToPaste).subscribe((tt: any) => {
